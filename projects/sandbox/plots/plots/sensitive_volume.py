...
@scriptify
def main(
    background_files: Iterable[Path],
    foreground_files: Iterable[Path],
    rejected_params: Iterable[Path],
    output_fname: Path,
    ...
    for background_file, foreground_file, rejected_file in zip(
        background_files, foreground_files, rejected_params
    ):
        logging.info(f"\t{background_file}")
        logging.info(f"\t{foreground_file}")
        logging.info(f"\t{rejected_file}")
        background = TimeSlideEventSet.read(background_file)
        foreground = RecoveredInjectionSet.read(foreground_file)
        rejected_params = InjectionParameterSet.read(rejected_file)
